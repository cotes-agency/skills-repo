---
name: skill-creator
description: Guide for creating effective skills. This skill should be used when users want to create a new skill (or update an existing skill) that extends Google Antigravity or Gemini CLI's capabilities with specialized knowledge, workflows, or tool integrations.
---
...
**Important - Choose the Correct Directory (Scope):**
When creating a skill, you can place it in a local (workspace) scope or a global (user) scope.

*Local Scope (Current Workspace Only):*
- **Antigravity (Editor Chat):** Use `.agent/skills` (e.g., `--path .agent/skills`).
- **Gemini CLI (Interactive Session):** Use `.agents/skills` (e.g., `--path .agents/skills`).

*Global Scope (Available Everywhere):*
- **Antigravity (Editor Chat):** Use `~/.gemini/antigravity/skills` (e.g., `--path ~/.gemini/antigravity/skills`).
- **Gemini CLI (Interactive Session):** Use `~/.gemini/agents/skills` (e.g., `--path ~/.gemini/agents/skills`).

> [!TIP]
> Most users prefer to keep both local environments in sync. See Step 6 for tips on using symlinks to share skills between both local environments.
...
If the user agrees to an installation, perform it immediately using the `run_command` tool:
...
**Important:** After the installation is complete, notify the user that they must verify or reload the skills depending on their environment:

1. **Interactive Gemini CLI Session**:
   - Uses `.agents/skills`.
   - They MUST manually execute the `/skills reload` command to enable the new skill.
   - Verify with `/skills list`.
2. **Antigravity (Editor Chat)**:
   - Uses `.agent/skills`.
   - New skills are usually detected automatically on the next turn.
   - If a new skill is not detected, you do not need to run a reload command (commands like `gemini skills reload` only affect the CLI).
   - To force detection, either open a **new conversation** or **explicitly ask the agent** to use the skill by name.

> [!TIP]
> **Keep your environments in sync:** If you want Antigravity and the Gemini CLI to share the same skills, create a symlink between the two directories:
> ```bash
> # Example: linking .agents to .agent
> ln -s .agent/skills .agents/skills
> ```

Note: You (the agent) cannot execute the `/skills reload` command or slash commands yourself; they must be done by the user in an interactive session. However, you CAN execute `gemini skills list` in the terminal to verify the installation on their behalf.
